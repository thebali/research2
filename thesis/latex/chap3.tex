...
In collapsed Gibbs sampling, certain variables are marginalized out of the model. Griﬃths and Steyvers propose collapsed Gibbs sampling for LDA, with both $\theta$ and $\Phi$ marginalized. Only $z_{dn}$ is sampled, and the sampling is done conditioned on α, β and the topic assignments of other words $\hat{z{dn}}$.
