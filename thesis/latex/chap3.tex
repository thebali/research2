...
\textsc{Choose the term probabilities for each topic} 
The distribution of terms for each topic $i$ is represented as a multinomial distribution $\Phi_i$ , which is drawn from a symmetric Dirichlet distribution with parameter $\beta$.
...
; \qquad p ( \theta _ { d } | \alpha ) = \frac { \Gamma ( \sum _ { i = 1 } ^ { K } \alpha _ { i } ) } { \prod _ { i = 1 } ^ { K } \Gamma ( \alpha _ { i } ) } \prod _ { i = 1 } ^ { K } \theta _ { d i } ^ { \alpha _ { i } - 1 }
