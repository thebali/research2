...
Suppose there are $N$ users $U = \{u_1,u_2, \ldot ,u_N\},$ $M$ items
V = {v1, v2, · · · , vM}, a set of observed indices Q = {i, j}, where {ui, vj} ∈ U ×V defines the observed ratings X = {xi,j}, each of which is optionally associated with a reviewri,j = {w|w ⊂ V } of length Li,j , where V is the set of vocabulary used in the re- viewtext. Alternatively, let Uj denote the indices of users who have rated item vj. LetK denote the number of topics.
