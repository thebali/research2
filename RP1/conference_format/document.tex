...
% \documentclass[conference]{../sty/IEEEtrans
...
% no \IEEEPARstart
\lettrine[lines=2, findent=1pt, nindent=0pt]{W}e are living in the age of technology and ``computer-human interaction''. In fact computer-human interaction has gone past the human-human interaction. The day is not far from when we will be having complete human-computer interface.
...
another one below for user similarity \eqref{eq:UserSim}...

%% pearson correlation for the user similarity
\begin{figure*}[b]
\begin{equation}
...
Receiver operating characteristic (ROC) curve is the graph of true positive rate versus false positive rate. Area Under ROC curve (AUC), A is defined as the following integral \cite{Bamber1975}:

\begin{equation}
\includegraphics[width=0.4 \textwidth]{images/AUC}
\label{eq:AUC} \tag{7}
\end{equation}
where $p_{f(x^-)}(c)$ is the probability density function of the random variable $f(x^-)$ at point $c$.
...
\nocite{Claypool1999}
\nocite{JonathanL.HerlockerJosephA.KonstanAlBorchers1999}
\nocite{Breese1998}
\nocite{Park2012}
\nocite{Adomavicius2005}
\nocite{Zaslow2002}
\nocite{Koren2009}
\nocite{Mustafa2017}
\nocite{Pagano2017}
\nocite{Kluver2014}
\nocite{sarwar2001item}
\nocite{Ekstrand2011}
\nocite{Cremonesi2010}
\nocite{Wei2017}
\nocite{Bamber1975}
