...
\begin{equation}
\begin{split} \text{where } R_{u,i} \text{ and }  R_{u,j} \text{ represent the rating of user } u \\ \text{ on items } i \text{ \& } j \text{ respectively,  } \overline{R}_u \text{ is the mean of the} \\ \text{ u\textsuperscript{th} user's ratings and } U_{i,j} \text{ represents all users who have rated items } i  \text{ \& } j.
\end{split}
\end{equation}
