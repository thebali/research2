...
\begin{figure*}[b]
\begin{equation}
ItemSim(x, y) = \frac{\sum_{u \in U_{i,j}} ({R_{u,i} - \overline{R}_u}) ({R_{u,j} - \overline{R}_u})} {\sqrt{\sum_{u \in U_{i,j}}({R_{u,i} - \overline{R}_u})^2} \sqrt{\sum_{u \in U_{i,j}}({R_{u,j} - \overline{R}_u})^2}}
\label{eq:ItemSim} \tag{4}
\end{equation}

\end{figure*}

\begin{multiline} \text{where } R_{u,i} \text{ and }  R_{u,j} \text{ represent the rating of user } u \text{ on items } i \text{ \& } j \text{ respectively,  } \\ \overline{R}_u \text{ is the mean of the u\textsuperscript{th} user's ratings and } U_{i,j} \text{ represents all users who have rated items } i  \text{ \& } j.\end{multiline}
